// /client/statements — single ledger + monthly statement surface.
//
// Replaces the old split between /client/statements (pick-a-month
// PDF download) and /client/transactions (full filterable ledger).
// The two were the same data viewed two ways; merging them removes
// a confusing nav choice and gives the client one place to either
...
const PAGE_SIZE = 50;
// Clients almost never need to scroll past two years of history.
...
export default async function ClientStatementsPage({
  ...
  const tableWhere = buildWhere(client.id, requested, propertyId);
    ...
    <div className="space-y-6">
      ...
      {/* Summary callout. Two flavours:
          ...
          - "All months" → show the running 24-month totals so
            the client gets *some* headline answer at the top of
            the page even when they haven't narrowed down.       */}
      {selected ? (
        <SelectedMonthSummary
          period={selected}
          bucket={selectedBucket}
          downloadHref={`/client/statements/${selected.year}/${selected.month}`}
        />
      ) : (
    ...
    <section className="rounded-lg border border-stone-200 bg-white p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {/* Download is always offered for a picked month. The
            ...
            than disabling the button and leaving the client
            wondering why. */}
        <Link
          href={downloadHref}
          target="_blank"
...
function AllMonthsSummary({ byMonth }: { byMonth: Map<string, Bucket> }) {
  ...
  // the client sees the "since I joined" headline at the top of
...
function Pagination({
  ...
  const link = (p: number) => {
    ...
    return `/client/statements?${params.toString()}`;
  };
  return (
