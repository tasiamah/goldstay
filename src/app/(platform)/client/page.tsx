...
import { EarningsOverview } from "@/components/client/EarningsOverview";
import { HelpHint } from "@/components/client/HelpHint";
import { computeSetupChecklist } from "@/lib/client/setup-status";
import { listPayoutMethodsFor } from "@/lib/payouts";
import { syncClientNotifications } from "@/lib/notifications/sync";
...
export default async function ClientDashboardPage() {
  // requireClient handles the unmatched-user case by redirecting to
  // /client/pending, so by the time we get here we always have a real
  // Client row to render.
  ...
  const [
    ...
    // The lease list per client is small (single-digit to low-tens for
    // even our largest landlords), so taking the rows is cheaper than
    // a separate count + groupBy.
    prisma.lease.findMany({
      where: {
        unit: { property: { clientId: client.id } },
        status: "ACTIVE",
      },
    ...
    // Outstanding management agreements awaiting the client's signature.
    ...
    // one row each so even a client with hundreds of transactions
  ...
  const primary = pickPrimaryCurrency(summaries, client.preferredCurrency);
  ...
  // don't FX-convert other currencies into that primary — clients
  ...
  const showFirstVisitHint = !client.welcomeCompletedAt;

  return (
    <div className="space-y-10">
      {showFirstVisitHint ? (
        <FirstVisitHint
          clientFirstName={client.fullName.split(/\s+/)[0] || "there"}
      ...
      {!setupComplete ? (
        ...
        // /client/account where the client is about to fill the
        // fields anyway, so duplicating it inside the banner just
        // made the dashboard noisier without giving the client a
            ...
            <Link
              href={
                setup.firstIncomplete
                  ? `/client/account?step=${setup.firstIncomplete}#${setup.firstIncomplete}`
      ...
      {pendingAgreements.length > 0 ? (
        <section className="rounded-lg border border-amber-200 bg-amber-50 p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <Link
              href={`/client/agreements/${pendingAgreements[0].id}`}
      ...
      {/* Headline earnings card. Anchors the dashboard at the
          ...
          single question every client opens this page to answer.
        ...
        {properties.length === 0 ? (
          ...
          <ul className="mt-4 divide-y divide-stone-100">
            {properties.map((p) => {
              ...
              // We have agreement-pending state client-wide; this
              ...
              return (
                <li key={p.id} className="py-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <PropertyReadinessBadge
                      status={p.status}
                      clientSideDone={readiness.clientSideDone}
                    />
                  </div>
        ...
        <section className="rounded-lg border border-stone-200 bg-white p-6">
          <div className="flex items-center gap-2">
            ...
            <HelpHint label="By currency">
              Clients with portfolios across Kenya, Ghana and the
      ...
      {/* Recent activity — drill-down ledger preview. Full width
          because it's the last section before /client/statements,
            ...
            {recentTransactions.map((t) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-stone-500">
                    <Link
                      href={`/client/properties/${t.property.id}`}
