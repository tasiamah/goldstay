// /client/pending — landing surface for landlords who are signed in
// to Supabase but not yet linked to a Client row in our DB. Hit when:
//   1. A new landlord lands on /login before Goldstay has manually
//      created their Client row (typo'd email at signup, onboarding
//      paperwork still pending, etc.)
//   2. A signed-in user tries to deep-link to a portal route their
//      account isn't entitled to. requireClient() redirects them here
//      rather than to /client so the pending state is the same URL
//      every time.
//
// Intentionally not gated by requireClient() (which would loop). It is
    ...
    <div className="mx-auto max-w-2xl py-6">
      <div className="rounded-lg border border-stone-200 bg-white p-8 shadow-sm">
        ...
        <p className="mt-3 text-stone-600">
          Goldstay onboards every client by hand so the right title
