...
export function YourDetailsForm({
  ...
  // identity and reassign the supabase user, which the client
  ...
  const [entityType, setEntityType] = useState<ClientEntityType>(
    defaultEntityType,
  );
  ...
  return (
    ...
    >
      ...
      {entityType === "COMPANY" ? (
        ...
        // value (we keep the client's existing country) and an explicit
        // empty companyName / registration number. Saves a second
        // round-trip when the client toggles back from COMPANY.
        <>
...
function EntityTypeOption({
  ...
  hint: string;
  selected: boolean;
  onSelect: (v: ClientEntityType) => void;
}) {
  return (
    <button
      type="button"
