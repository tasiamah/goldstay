...
// One slot in the client KYC card. Renders either:
...
// also means a client who wants to swap a smudged photo doesn't have
// to hunt for the file picker.

import { useRef, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import {
  clientCreateKycUploadAction,
  clientFinaliseKycUploadAction,
} from "./kyc-actions";
...
export type ClientKycSlotProps = {
  kind: SlotKind;
  title: string;
  description: string;
  existing: {
...
export function ClientKycSlot({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setUploading(true);
    try {
      const init = await clientCreateKycUploadAction({
        kind,
        filename: file.name,
      ...
      const fin = await clientFinaliseKycUploadAction({
