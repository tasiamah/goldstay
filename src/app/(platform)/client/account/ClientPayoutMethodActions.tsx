...
import {
  clientArchivePayoutMethodAction,
  clientSetDefaultPayoutMethodAction,
} from "./actions";

export function ClientPayoutMethodActions({
  payoutMethodId,
  isDefault,
}: {
          ...
          <button
            ...
            onClick={() =>
              startTransition(async () => {
                setError(null);
                const r = await clientSetDefaultPayoutMethodAction(payoutMethodId);
        ...
        <button
          ...
          onClick={() =>
            startTransition(async () => {
              ...
              if (!ok) return;
              const r = await clientArchivePayoutMethodAction(payoutMethodId);
              if (!r.ok) setError(r.error);
              else router.refresh();
            })
          }
