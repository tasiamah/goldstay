// /client/account — single page for the entire client setup.
//
// Replaces the previous /client/profile + /client/payouts split. The
...
//
// /client/payouts and /client/profile both redirect to this page so
...
import type { SetupStepKey } from "@/lib/client/setup-status";
import { ClientPayoutMethodActions } from "./ClientPayoutMethodActions";
  ...
  return (
    <div className="space-y-8">
      ...
      <SetupChecklist
        data={checklist}
        activeKey={activeKey}
        hrefFor={(key) => `/client/account?step=${key}#${key}`}
      />
      ...
      >
        <YourDetailsForm
          defaultFullName={client.fullName}
          defaultPhone={client.phone ?? ""}
          defaultAddress={client.address ?? ""}
          defaultEntityType={client.entityType}
          defaultCompanyName={client.companyName ?? ""}
      ...
      >
        <ClientKycCard clientId={client.id} />
      </SetupSection>
      ...
      >
        ...
        <div className="mt-6 rounded-lg border border-stone-200 bg-white p-6">
          ...
          <div className="mt-4">
            <ClientAddPayoutMethodForm
              defaultCurrency={client.preferredCurrency}
            />
          </div>
        </div>
      </SetupSection>
