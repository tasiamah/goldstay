...
// Client-side add-payout-method form. Same conditional-fields shape
// as the admin one, but with a friendlier voice (no "internal
// notes" textarea, more help text). Submits via the
// client-scoped action that requires requireClient() server-side.

import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import type { PayoutMethodKind } from "@prisma/client";
import { clientCreatePayoutMethodAction } from "./actions";

export function ClientAddPayoutMethodForm({
  defaultCurrency,
}: {
    ...
    <form
      action={(formData) => {
        ...
        startTransition(async () => {
          const r = await clientCreatePayoutMethodAction(null, formData);
          if (!r.ok) {
            setError(r.error);
