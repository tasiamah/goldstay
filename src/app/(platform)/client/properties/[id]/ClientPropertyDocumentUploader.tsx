...
// Client self-serve property document upload form.
//
...
import {
  clientCreatePropertyDocumentUploadAction,
  clientFinalisePropertyDocumentUploadAction,
  type ClientPropertyDocKind,
...
export function ClientPropertyDocumentUploader({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setUploading(true);
    try {
      const init = await clientCreatePropertyDocumentUploadAction({
        propertyId,
        kind,
      ...
      const fin = await clientFinalisePropertyDocumentUploadAction({
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block text-sm">
          ...
          <select
            value={kind}
            onChange={(e) =>
              setKind(e.target.value as ClientPropertyDocKind)
            }
            disabled={busy}
