...
// Per-row controls for a property document on the client page.
// Currently just a Remove button, surfaced only when the doc is
// client-uploaded AND not yet verified by Goldstay (the action
// re-checks this server-side, so a stale render can never delete a
// verified row through here).

import { useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import { clientDeletePropertyDocumentAction } from "./document-actions";

export function ClientPropertyDocumentRow({
  documentId,
  title,
}: {
  documentId: string;
      ...
      <button
        ...
        onClick={() => {
          ...
          startTransition(async () => {
            setError(null);
            const r = await clientDeletePropertyDocumentAction(documentId);
            if (!r.ok) {
              setError(r.error);
              return;
            }
