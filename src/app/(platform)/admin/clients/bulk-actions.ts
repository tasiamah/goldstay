...
// Bulk archive ids submitted from the clients list page. Failures
...
export async function bulkArchiveClientsAction(formData: FormData): Promise<{
  archived: number;
  failed: number;
}> {
  ...
  for (const id of ids) {
    try {
      await archiveEntity("CLIENT", id, actor);
      archived += 1;
    } catch (err) {
      console.warn("[bulk-archive-clients] failed", id, err);
      failed += 1;
    }
  }
