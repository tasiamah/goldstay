// /admin/clients/[id]/payouts/record — single-form page for an
// admin to record a money-out PAYOUT against a verified
// ClientPayoutMethod. The dropdowns are pre-filtered to only show
// the client's properties + their verified, non-archived methods,
...
export default async function RecordPayoutPage({
  ...
  const [properties, methods] = await Promise.all([
    ...
    listPayablePayoutMethods(client.id),
  ]);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-stone-500">
          ...
          The client sees this on their next statement and on the
      ...
      {methods.length === 0 ? (
        <div className="rounded-lg border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900">
          <p className="font-medium">
            No verified payout method on file for this client.
          </p>
          <p className="mt-1">
            Verify a payout method on the client detail page before
            recording any payouts.
          </p>
        </div>
      ) : properties.length === 0 ? (
        <div className="rounded-lg border border-stone-200 bg-white p-5 text-sm text-stone-600">
          This client has no active properties, so there&apos;s nothing to
