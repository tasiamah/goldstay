// Server component rendering the client's payout methods + an
...
// document on the client). Default is exactly one row, marked with
...
export async function PayoutMethodsCard({
  ...
  const methods = await listPayoutMethodsFor(clientId, { includeArchived: false });
    ...
    <div className="rounded-lg border border-stone-200 bg-white p-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">
            Where Goldstay remits the client&apos;s net rent each month.
      ...
      {methods.length === 0 ? (
        <p className="mt-5 rounded-md border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-stone-600">
          ...
          client can be paid out programmatically.
        </p>
      ) : (
