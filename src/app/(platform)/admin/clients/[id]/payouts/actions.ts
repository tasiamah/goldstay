...
export async function createPayoutMethodAction(
  ...
  await requireRole("client.write");
...
// ClientPayoutMethod. Refuses if the method is unverified, archived,
// or doesn't belong to the property's client — these are the three
...
export async function recordPayoutAction(
  ...
  const [property, method] = await Promise.all([
    ...
    prisma.clientPayoutMethod.findUnique({
  ...
  if (!property || property.clientId !== clientId) {
    return {
      ok: false,
      error: "Property does not belong to this client.",
      fieldErrors: { propertyId: "Wrong client" },
    };
  }
  if (!method || method.clientId !== clientId) {
    return {
      ok: false,
      error: "Payout method does not belong to this client.",
      fieldErrors: { payoutMethodId: "Wrong client" },
    };
  }
  ...
  try {
    ...
    await Promise.all([
      ...
      recordAudit({
        ...
        metadata: { transactionId: created.id, clientId, propertyId: created.propertyId },
      }),
      recordAudit({
        actor,
        entity: "CLIENT",
