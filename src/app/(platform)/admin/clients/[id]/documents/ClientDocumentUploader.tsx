...
// Mirror of the property-document uploader, scoped to the client.
...
// limited to genuinely client-level documents.

import { useRef, useState, useTransition } from "react";
import { useRouter } from "next/navigation";
import {
  createClientDocumentUploadAction,
  finaliseClientDocumentUploadAction,
} from "./actions";

type ClientDocumentKindValue =
  | "ID_DOCUMENT"
  | "KYC"
  | "PROOF_OF_PAYOUT_ACCOUNT"
  | "OTHER";

const KIND_OPTIONS: { value: ClientDocumentKindValue; label: string }[] = [
...
export function ClientDocumentUploader({ clientId }: { clientId: string }) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [title, setTitle] = useState("");
  const [kind, setKind] = useState<ClientDocumentKindValue>("ID_DOCUMENT");
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setUploading(true);
    try {
      const init = await createClientDocumentUploadAction({
        clientId,
        title: title.trim(),
        kind,
        filename: file.name,
      ...
      const fin = await finaliseClientDocumentUploadAction({
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <label className="block text-sm">
          ...
          <select
            value={kind}
            onChange={(e) => setKind(e.target.value as ClientDocumentKindValue)}
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {busy ? "Uploading…" : "Upload client document"}
        </button>
        <p className="text-xs text-stone-500">
          Goes to encrypted storage. Visible to the client via their
