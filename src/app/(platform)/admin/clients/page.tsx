import Link from "next/link";
import type { Prisma } from "@prisma/client";
import { prisma } from "@/lib/db";
import {
  formatClientDisplayName,
  formatClientSecondaryName,
} from "@/lib/format-client";
import { ListSearchBar } from "@/components/admin/ListSearchBar";
import { FilterSelect } from "@/components/admin/FilterSelect";
import {
  ActiveFilterChips,
  type Chip,
} from "@/components/admin/ActiveFilterChips";
import {
  PERIOD_LABEL,
  parseClientListFilters,
  periodRange,
  toQueryString,
  type RawSearchParams,
} from "@/lib/admin/list-search";
import {
  PlainHeader,
  SortableHeader,
} from "@/components/admin/table/SortableHeader";
import { Pagination } from "@/components/admin/table/Pagination";
import {
  parsePagination,
  parseSort,
  sortToParam,
  type SortState,
} from "@/lib/admin/table";
import { BulkActionBar } from "./BulkSelectClient";

export const dynamic = "force-dynamic";

const SORTABLE_CLIENT_COLUMNS = [
  "fullName",
  "email",
  "country",
  "createdAt",
] as const;

export default async function ClientsListPage({
  searchParams,
}: {
  searchParams?: RawSearchParams;
}) {
  const filters = parseClientListFilters(searchParams);
  const rawParams = (searchParams ?? {}) as Record<string, string>;
  const sort = parseSort(
    rawParams.sort,
    SORTABLE_CLIENT_COLUMNS,
    { column: "createdAt", direction: "desc" },
  );
  const { page, pageSize } = parsePagination(rawParams);

  // Build a Prisma where clause from the parsed filters. Search
  // covers the four fields a Goldstay ops person reasonably types
  // when looking for a landlord: name, company, email, phone.
  const where: Prisma.ClientWhereInput = { archivedAt: null };
  if (filters.country) where.country = filters.country;
  // Period drills the list to clients created in a specific calendar
  // month. Used by the admin overview "New clients" KPI deep-link so
  // clicking the number shows the rows that produced it.
  const range = periodRange(filters.period);
  if (range) where.createdAt = { gte: range.gte, lt: range.lt };
  if (filters.q) {
    where.OR = [
      { fullName: { contains: filters.q, mode: "insensitive" } },
      { companyName: { contains: filters.q, mode: "insensitive" } },
      { email: { contains: filters.q, mode: "insensitive" } },
      { phone: { contains: filters.q, mode: "insensitive" } },
    ];
  }

  const orderBy: Prisma.ClientOrderByWithRelationInput = {
    [sort.column]: sort.direction,
  } as Prisma.ClientOrderByWithRelationInput;

  const [clients, filteredCount, totalCount] = await Promise.all([
    prisma.client.findMany({
      where,
      orderBy,
      skip: (page - 1) * pageSize,
      take: pageSize,
      include: { _count: { select: { properties: true } } },
    }),
    prisma.client.count({ where }),
    prisma.client.count({ where: { archivedAt: null } }),
  ]);

  const isFiltered =
    filters.q !== "" || filters.country !== null || filters.period !== null;
  const chips: Chip[] = [];
  if (filters.q) chips.push({ key: "q", label: `Search: “${filters.q}”` });
  if (filters.country)
    chips.push({
      key: "country",
      label: filters.country === "KE" ? "Kenya" : "Ghana",
    });
  if (filters.period)
    chips.push({ key: "period", label: PERIOD_LABEL[filters.period] });

  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-medium text-stone-900">Clients</h2>
          <p className="text-sm text-stone-500">
            {isFiltered ? (
              <>
                <strong className="text-stone-700">{filteredCount}</strong> of{" "}
                {totalCount} {totalCount === 1 ? "client" : "clients"} match
              </>
            ) : (
              <>
                {totalCount} {totalCount === 1 ? "client" : "clients"} in the
                platform
              </>
            )}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Link
            href={`/admin/clients/export${toQueryString({
              q: filters.q,
              country: filters.country,
              period: filters.period,
            })}`}
            className="inline-flex items-center rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-stone-700 hover:bg-stone-50"
          >
            Export CSV
          </Link>
          <Link
            href="/admin/clients/import"
            className="inline-flex items-center rounded-md border border-stone-300 bg-white px-3 py-2 text-sm font-medium text-stone-700 hover:bg-stone-50"
          >
            Import CSV
          </Link>
          <Link
            href="/admin/clients/new"
            className="inline-flex items-center rounded-md bg-stone-900 px-4 py-2 text-sm font-medium text-white hover:bg-stone-800"
          >
            Add client
          </Link>
        </div>
      </div>

      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-1 flex-wrap items-center gap-3">
          <ListSearchBar
            placeholder="Search name, company, email, phone…"
            ariaLabel="Search clients"
          />
          <FilterSelect
            name="country"
            label="Country"
            options={[
              { value: "KE", label: "Kenya" },
              { value: "GH", label: "Ghana" },
            ]}
          />
        </div>
      </div>

      <ActiveFilterChips
        chips={chips}
        basePath="/admin/clients"
        allParams={{
          q: filters.q,
          country: filters.country,
          period: filters.period,
        }}
      />

      {clients.length === 0 ? (
        isFiltered ? (
          <NoMatches resetHref="/admin/clients" />
        ) : (
          <EmptyState />
        )
      ) : (
        <form id="clients-bulk-form" className="overflow-x-auto rounded-lg border border-stone-200 bg-white">
          <table className="min-w-full divide-y divide-stone-200">
            <thead className="bg-stone-50">
              <tr>
                <PlainHeader>
                  <span className="sr-only">Select</span>
                </PlainHeader>
                <SortableHeader
                  column="fullName"
                  label="Name"
                  current={sort}
                  basePath="/admin/clients"
                  params={clientsTableParams(filters, sort, pageSize)}
                />
                <SortableHeader
                  column="email"
                  label="Email"
                  current={sort}
                  basePath="/admin/clients"
                  params={clientsTableParams(filters, sort, pageSize)}
                />
                <SortableHeader
                  column="country"
                  label="Country"
                  current={sort}
                  basePath="/admin/clients"
                  params={clientsTableParams(filters, sort, pageSize)}
                />
                <PlainHeader align="right">Properties</PlainHeader>
                <SortableHeader
                  column="createdAt"
                  label="Joined"
                  current={sort}
                  basePath="/admin/clients"
                  params={clientsTableParams(filters, sort, pageSize)}
                />
              </tr>
            </thead>
            <tbody className="divide-y divide-stone-100">
              {clients.map((o) => {
                const secondary = formatClientSecondaryName(o);
                return (
                  <tr key={o.id} className="hover:bg-stone-50/60">
                    <td className="px-4 py-3 align-top">
                      <input
                        type="checkbox"
                        name="ids"
                        value={o.id}
                        aria-label={`Select ${formatClientDisplayName(o)}`}
                        className="h-4 w-4 rounded border-stone-300 text-stone-900 focus:ring-stone-500"
                      />
                    </td>
                    <td className="px-4 py-3">
                      <Link
                        href={`/admin/clients/${o.id}`}
                        className="font-medium text-stone-900 hover:underline"
                      >
                        {formatClientDisplayName(o)}
                      </Link>
                      {secondary ? (
                        <p className="text-xs text-stone-500">{secondary}</p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3 text-sm text-stone-700">
                      <a
                        href={`mailto:${o.email}`}
                        className="hover:underline"
                      >
                        {o.email}
                      </a>
                    </td>
                    <td className="px-4 py-3 text-sm text-stone-700">
                      {o.country === "KE" ? "Kenya" : "Ghana"}
                    </td>
                    <td className="px-4 py-3 text-right text-sm tabular-nums text-stone-700">
                      {o._count.properties}
                    </td>
                    <td className="px-4 py-3 text-sm text-stone-500">
                      {o.createdAt.toLocaleDateString("en-GB", {
                        day: "2-digit",
                        month: "short",
                        year: "numeric",
                      })}
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
          <Pagination
            basePath="/admin/clients"
            params={clientsTableParams(filters, sort, pageSize)}
            page={page}
            pageSize={pageSize}
            totalRows={filteredCount}
          />
        </form>
      )}
      <BulkActionBar formId="clients-bulk-form" />
    </div>
  );
}

function clientsTableParams(
  filters: ReturnType<typeof parseClientListFilters>,
  sort: SortState,
  pageSize: number,
): Record<string, string> {
  const out: Record<string, string> = {
    sort: sortToParam(sort),
    pageSize: String(pageSize),
  };
  if (filters.q) out.q = filters.q;
  if (filters.country) out.country = filters.country;
  if (filters.period) out.period = filters.period;
  return out;
}

function EmptyState() {
  return (
    <div className="rounded-lg border border-dashed border-stone-300 bg-white p-10 text-center">
      <h3 className="text-base font-medium text-stone-900">No clients yet</h3>
      <p className="mt-1 text-sm text-stone-500">
        Add your first client to start tracking properties and leases.
      </p>
      <Link
        href="/admin/clients/new"
        className="mt-4 inline-flex items-center rounded-md bg-stone-900 px-4 py-2 text-sm font-medium text-white hover:bg-stone-800"
      >
        Add client
      </Link>
    </div>
  );
}

function NoMatches({ resetHref }: { resetHref: string }) {
  return (
    <div className="rounded-lg border border-dashed border-stone-300 bg-white p-10 text-center">
      <h3 className="text-base font-medium text-stone-900">
        No clients match those filters
      </h3>
      <p className="mt-1 text-sm text-stone-500">
        Try a shorter search or remove a filter.
      </p>
      <Link
        href={resetHref}
        className="mt-4 inline-flex items-center rounded-md border border-stone-300 bg-white px-4 py-2 text-sm font-medium text-stone-700 hover:bg-stone-50"
      >
        Clear all filters
      </Link>
    </div>
  );
}
