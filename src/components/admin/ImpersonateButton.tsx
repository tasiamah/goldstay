import { startImpersonationAction } from "@/app/(platform)/admin/clients/[id]/impersonate-actions";
...
//
// We gate the render on `adminCan("impersonate.client")` so SUPPORT
...
export async function ImpersonateButton({ clientId }: { clientId: string }) {
  const allowed = await adminCan("impersonate.client");
  if (!allowed) return null;

  const action = startImpersonationAction.bind(null, clientId);
  return (
