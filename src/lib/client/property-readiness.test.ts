...
//   * an ONBOARDING property with everything client-side done falls
//     back to "Goldstay review" so the client is never left guessing
...
describe("computePropertyReadiness", () => {
  ...
  it("surfaces both client-side blockers in canonical order", () => {
    const r = computePropertyReadiness({
  ...
  it("falls back to Goldstay review when the client side is fully done", () => {
    const r = computePropertyReadiness({
      propertyStatus: "ONBOARDING",
