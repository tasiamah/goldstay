...
// /client/payouts and the dashboard "finish setup" callout. The risk
// that matters: a step is marked done while the underlying data is
// still missing → the client thinks they're cleared and we hit a
...
//   * a COMPANY-mode client with no companyName fails Details
//   * an INDIVIDUAL-mode client doesn't need companyName for Details
...
describe("computeSetupChecklist", () => {
  it("treats a complete individual client as 3 of 3 with no firstIncomplete", () => {
  ...
  it("rejects a single-token name, a phone-less client, or a missing address on Details", () => {
    expect(
      computeSetupChecklist({
        ...baseInput,
        client: { ...baseInput.client, fullName: "Asha" },
      }).firstIncomplete,
    ).toBe("details");
    expect(
      computeSetupChecklist({
        ...baseInput,
        client: { ...baseInput.client, phone: "" },
      }).firstIncomplete,
    ).toBe("details");
    expect(
      computeSetupChecklist({
        ...baseInput,
        client: { ...baseInput.client, address: null },
      }).firstIncomplete,
    ).toBe("details");
  });
