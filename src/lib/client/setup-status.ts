// Computes which of the three client-onboarding steps are complete.
//
// Pure: takes a snapshot of the client row, their KYC documents and
// their payout methods, returns a three-row checklist. Same fixture
// can drive both the visual <SetupChecklist> and any "is this client
...
//                 step (companyName defaults to the client's name at
//                 record creation).
...
//                 "verified" separately so a client can still tick
...
export function computeSetupChecklist(input: SetupInput): SetupChecklist {
  const personalCore =
    isFullName(input.client.fullName) &&
    Boolean(input.client.phone?.trim()) &&
    Boolean(input.client.address?.trim());
  const businessOk =
    input.client.entityType === "COMPANY"
      ? Boolean(input.client.companyName?.trim())
      : true;
